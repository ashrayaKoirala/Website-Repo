...
import { Scissors, Upload, FileText, X, Check, Info, Play, Download } from "lucide-react"
...
const VideoCutter: React.FC = () => {
  ...
  const handleProcessVideo = () => {
    if (!videoFile || !cutProfileFile) {
      setErrorDetails({ title: "Missing Files", message: "Please upload both video and cut profile files.", details: "" })
    ...
    WorkersApi.cutVideo(videoFile, cutProfileFile)
      .then((response) => {
      ...
      .catch((err) => {
        ...
        setErrorDetails({
          ...
          message: err?.message || "An unexpected error occurred while cutting the video.",
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600 mb-2">Cut video automatically based on generated cut-profiles.</p>
        <Dialog.Root open={isHowItWorksOpen} onOpenChange={setIsHowItWorksOpen}>
          ...
          <Dialog.Portal>
            ...
            <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90vw] max-w-md bg-white p-6 rounded-lg shadow-lg data-[state=open]:animate-contentShow focus:outline-none z-50">
              <Dialog.Title className="text-lg font-medium mb-2">How Video Cutter Works</Dialog.Title>
              <Dialog.Description className="text-sm text-gray-600 mb-4">
                Placeholder text: Upload your original video file and the corresponding JSON cut profile
                generated by the Cut Profile Generator. The worker processes the video according to the
                cut instructions in the profile. Download the resulting shorter video.
              </Dialog.Description>
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden p-6">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="flex justify-center mb-8">
            {status === 'processing' ? (
              ...
              <button
                ...
                disabled={!videoFile || !cutProfileFile || status === 'processing'}
                className={`py-3 px-8 rounded-lg flex items-center ${
                  !videoFile || !cutProfileFile || status === 'processing'
                    ? "bg-gray-300 text-gray-500 cursor-not-allowed"
                    : "bg-black text-white hover:bg-gray-800"
                } transition-colors`}
                aria-disabled={!videoFile || !cutProfileFile || status === 'processing'}
              >
          ...
          {/* Result Preview - shown only on success */}
